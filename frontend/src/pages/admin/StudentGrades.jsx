        ...
        <ProtectedRoute roles={['admin']}>
            <div className="space-y-8 pb-24">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-xl font-semibold tracking-tight text-slate-900">Grade Change for Multiple Students</h1>
                        <p className="text-slate-500 text-sm mt-1">Efficiently update grade levels for multiple students in a single action.</p>
                    </div>
                    {students.length > 0 && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
                        ...
                        <div className="md:col-span-5 p-6 space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-semibold text-slate-900 uppercase tracking-wide">From Grade</span>
                            </div>
                            <CustomDropdown
                                icon={School}
                                value={sourceGrade}
                                onChange={(val) => setSourceGrade(val)}
                            ...
                            />
                            <p className="text-xs text-slate-500 pl-1">Select the grade you want to move students <b>from</b>.</p>
                        </div>

                        {/* Middle Arrow */}
                        <div className="md:col-span-2 flex items-center justify-center py-4 md:py-0 relative">
                            <div className="absolute inset-x-0 top-1/2 border-t border-slate-100 hidden md:block"></div>
                            <div className="w-10 h-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center z-10 text-slate-400">
                                <ArrowRight size={20} />
                            </div>
                        </div>

                        {/* Target Column */}
                        <div className="md:col-span-5 p-6 space-y-4 bg-slate-50/50 rounded-r-xl">
                            <div className="flex items-center gap-2 mb-2">
                                <div className="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xs font-bold">2</div>
                                <span className="text-sm font-semibold text-slate-900 uppercase tracking-wide">To Grade</span>
                            </div>
                            <CustomDropdown
                                icon={GraduationCap}
                                value={targetGrade}
                                ...
                                options={grades.filter(g => g.id.toString() !== sourceGrade).map(grade => ({ value: String(grade.id), label: grade.name }))}
                            />
                            <p className="text-xs text-slate-500 pl-1">Select the destination grade to move students <b>to</b>.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    {!sourceGrade ? (
                        <div className="flex flex-col items-center justify-center p-12 text-slate-400 animate-in fade-in zoom-in-95 leading-relaxed">
                            <div className="w-20 h-20 bg-slate-50 rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100">
                                <Users size={32} className="text-slate-300" />
                            </div>
                            <h3 className="text-lg font-semibold text-slate-900">No Grade Selected</h3>
                            ...
                            <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                                ...
                                <div className="relative w-full sm:w-72">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:bg-white focus:ring-4 focus:ring-slate-100 focus:border-slate-300 transition-all"
                                    />
                                </div>
                            </div>
                                    ...
                                    <table className="min-w-full divide-y divide-slate-100">
                                        <thead className="bg-slate-50/50">
                                            <tr>
                                                <th className="px-6 py-4 w-16">
                                                    ...
                                                    >
                                                        {selectedStudents.size === filteredStudents.length && filteredStudents.length > 0 ? (
                                                            ...
                                                            <div className="text-slate-300 hover:text-slate-400">
                                                                <Square size={20} />
                                                            </div>
                                                        )}
                                                    </button>
                                                ...
                                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Guardian Name</th>
                                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Email</th>
                                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Language</th>
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="text-xs text-slate-500 mt-0.5">ID: {student.username || 'N/A'}</div>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            <div className="text-sm font-medium text-slate-700">
                                                                {student.guardian_name || <span className="text-slate-400 italic">Not set</span>}
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4">
            ...
            {/* Floating Action Bar */}
            <div className={`
                fixed bottom-8 left-1/2 -translate-x-1/2 bg-white rounded-xl shadow-lg border border-slate-100 p-2 pl-6 pr-2 flex items-center gap-8 transition-all duration-300 z-40
            ...
            `}>
                ...
                <button
                    ...
                    className="ml-4 bg-slate-900 text-white px-6 py-3 rounded-xl text-sm font-semibold hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 flex items-center gap-2"
                >
                    Confirm Move
                    <ArrowRight size={16} />
                </button>
            ...
            {showConfirmModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-lg w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 ring-1 ring-slate-900/5">
                        <div className="p-8">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-slate-900">Confirm Grade Transfer</h3>
                                    <p className="text-slate-500 text-sm mt-1">Review the details below before proceeding.</p>
                                </div>
                            </div>

                            <div className="bg-slate-50 rounded-xl p-4 space-y-3 border border-slate-100">
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500">Students Selected</span>
                                    <span className="text-sm font-bold text-slate-900">{selectedStudents.size}</span>
                                </div>
                                <div className="h-px bg-slate-200"></div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500">From Grade</span>
                                    <span className="text-sm font-medium text-slate-900">{getGradeName(sourceGrade)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-sm text-slate-500">To Grade</span>
                        ...
                        <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
                            <button
                                ...
                                className="flex-1 px-4 py-3 text-sm font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 transition-all shadow-sm"
                            >
                                Cancel
                            </button>
                            <button
