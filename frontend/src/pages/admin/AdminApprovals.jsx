...
const AdminApprovals = () => {
    ...
    const fetchPendingUsers = async () => {
        try {
            setLoading(true);
            const [usersRes, languagesRes] = await Promise.all([
                api.get('/admin/pending-users'),
                api.get('/system/languages')
            ]);

            const langs = languagesRes.data || [];
            const langMap = {};
            if (Array.isArray(langs)) {
                langs.forEach(l => langMap[l.id] = l.name);
            }

            const mappedUsers = (usersRes.data || []).map(user => ({
                ...user,
                preferred_language: user.preferred_language_id ? (langMap[user.preferred_language_id] || 'N/A') : (user.preferred_language || 'N/A')
            }));
        ...
        <ProtectedRoute roles={['admin']}>
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800 tracking-tight">New Users Permissions</h1>
                        <p className="text-gray-500 text-sm mt-1">Manage pending account approval requests.</p>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-64 text-gray-500">
                                ...
                                <p className="text-lg font-bold text-gray-900 mb-1">No pending approvals</p>
                                <p className="text-sm text-gray-600">All users have been processed.</p>
                            </div>
                        ) : (
                            ...
                            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p className="text-sm text-gray-500">
                                        Showing <span className="font-semibold text-gray-900">{indexOfFirstItem + 1}</span> to <span className="font-semibold text-gray-900">{Math.min(indexOfLastItem, filteredUsers.length)}</span> of{' '}
                                        <span className="font-semibold text-gray-900">{filteredUsers.length}</span> entries
                                    </p>
                                </div>
                                <div>
