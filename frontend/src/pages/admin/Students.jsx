...
const Students = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [studentsRes, gradesRes, languagesRes, schoolRes] = await Promise.all([
                api.get('/students'),
                api.get('/grades'),
            ...
            const allLangs = languagesRes.data || [];

            // Set dropdown options based on school allowances
            // Set dropdown options based on school allowances
            const schoolAllowed = schoolRes.data?.allowed_languages;
            let langsToUse = [];

            if (schoolAllowed && Array.isArray(schoolAllowed) && schoolAllowed.length > 0) {
                langsToUse = schoolAllowed;
            } else {
                langsToUse = allLangs;
            }

            // Sort alphabetically by name
            langsToUse.sort((a, b) => a.name.localeCompare(b.name));
            setLanguages(langsToUse);

            // Create a lookup for language names
            const langMap = {};
            if (Array.isArray(allLangs)) {
                allLangs.forEach(l => langMap[l.id] = l.name);
            }
            ...
            const mappedStudents = studentsRes.data
                ...
                .map(student => ({
                    ...
                    language: student.preferred_language_id ? (langMap[student.preferred_language_id] || 'N/A') : 'N/A',
