...
import { PASSWORD_REGEX, PASSWORD_MESSAGE, PASSWORD_REQUIREMENTS } from '../../../utils/validation';
...
const AdminResetPasswordModal = ({ userId, userName, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200 p-4">
            <div className="bg-white rounded-xl w-full max-w-md shadow-lg animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col">

                {/* Header */}
                <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Reset Password</h2>
                        <p className="text-xs text-slate-500 mt-1">For {userName}</p>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full pl-4 pr-10 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-white outline-none transition-all"
                        ...
                        {password && (
                            <ul className="text-xs space-y-1 pl-1">
                                {PASSWORD_REQUIREMENTS.map((req) => (
                                    <li key={req.label} className={req.test(password) ? 'text-green-600' : 'text-slate-400'}>
