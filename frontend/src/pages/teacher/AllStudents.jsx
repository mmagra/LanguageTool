...
const AllStudents = () => {
    ...
    const [languages, setLanguages] = useState([]); // Dynamic languages
        ...
        <ProtectedRoute roles={['teacher']}>
            <div className="space-y-6 animate-fade-in">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-semibold tracking-tight text-slate-900">View All Students</h1>
                        <p className="text-slate-500 text-sm mt-1">Select a student to send a private message.</p>
                    </div>
                </div>
                                    ...
                                    {currentStudents.map((student) => (
                                        <tr key={student.id} className="hover:bg-slate-100 transition-colors duration-200 group">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium overflow-visible">
                                                <button
                                                    onClick={() => navigate('/teacher/send-message', { state: { student } })}
